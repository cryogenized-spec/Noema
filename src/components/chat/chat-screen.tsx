...
import { OrganizerDraftReviewSheet } from "@/components/chat/organizer-draft-review-sheet";
...
import {
  buildChatConversionPrompt,
  buildChatOrganizerDraftEnvelope,
  mergeAiSuggestionIntoDraft,
  type ChatOrganizerDraftTarget,
} from "@/lib/organizer-intelligence/chat-conversion";
import { transitionOrganizerDraftReviewState } from "@/lib/organizer-intelligence/draft-output";
...
export function ChatScreen() {
  ...
  const [draftReview, setDraftReview] = useState<OrganizerDraftEnvelope | null>(null);
  ...
  const requestOrganizerAiSuggestion = async (target: ChatOrganizerDraftTarget, baseDraft: OrganizerDraftEnvelope) => {
    const runtimeRequest = buildOrganizerRuntimeRequest({
      invocation: {
        context: baseDraft.context,
        source: baseDraft.source,
        prompt: buildChatConversionPrompt(target, baseDraft.source),
      },
      agents,
    ...
    const routingAgent = agents.find((agent) => agent.id === runtimeRequest.request.selectedAgentId);
    if (!routingAgent) return baseDraft;

    if (!hasProviderAdapter(routingAgent.providerId)) {
      await addMessage({ role: "system", content: `Organizer draft AI is unavailable for ${routingAgent.providerId} right now.` });
      return baseDraft;
    }

    const provider = PROVIDER_CATALOG.find((item) => item.id === routingAgent.providerId);
    const model = provider?.models.find((item) => item.modelId === routingAgent.modelId);
    if (!provider || !model) return baseDraft;

    const providerConfig = records.find((record) => record.providerId === routingAgent.providerId);
    if (!providerConfig) return baseDraft;

    const apiKey = await revealKey(routingAgent.providerId);
    const built = buildAgentExecutionPayload({
      invocation: createInvocationDescriptor({
        threadId: activeThreadKey,
        sourceMessageContent: baseDraft.source.contentExcerpt,
        agentId: routingAgent.id ?? -1,
    ...
    if (!payload.content) return baseDraft;

    return mergeAiSuggestionIntoDraft(baseDraft, payload.content);
  };

  const beginOrganizerDraft = async (target: ChatOrganizerDraftTarget, sourceMessages: ChatMessage[]) => {
    if (sourceMessages.length === 0) return;
    setAgentPending(true);
    try {
      const baseDraft = buildChatOrganizerDraftEnvelope({
        target,
        messages: sourceMessages,
        selectedAgentId: activeAgentId,
      });
      const enhanced = await requestOrganizerAiSuggestion(target, baseDraft);
      setDraftReview(enhanced);
      await enqueueDrafts({
        drafts: [enhanced],
        source: "chat_conversion",
        sourceLabel: `Chat ${target} draft`,
        sourceRef: baseDraft.source.entityId ? String(baseDraft.source.entityId) : undefined,
      });
    } finally {
  ...
  const saveDraft = async (draft: OrganizerDraftEnvelope) => {
    setDraftSaving(true);
    try {
      if (draft.draftType === "task_draft" && "draft" in draft.payload) {
        ...
        await addMessage({ role: "system", content: "Task draft accepted and saved in Organizer Tasks." });
        ...
        await addMessage({ role: "system", content: "Document draft accepted and saved in Organizer Documents." });
        ...
        await addMessage({ role: "system", content: "Event draft accepted and saved in Organizer Calendar." });
      }
      setDraftReview(transitionOrganizerDraftReviewState(draft, "accepted"));
      setDraftReview(null);
      clearSelectionMode();
    } finally {
  ...
  const handleContextAction = async (action: ContextActionId, message: ChatMessage, context: MessagePressContext) => {
    ...
    if (action === "convert_task_draft") {
      await beginOrganizerDraft("task", [message]);
      return;
    }

    if (action === "convert_document_draft") {
      await beginOrganizerDraft("document", [message]);
      return;
    }

    if (action === "convert_event_draft") {
      await beginOrganizerDraft("event", [message]);
      return;
    }
    ...
    <section className="flex h-full min-h-0 flex-col gap-3">
      {selectionMode && (
        <SelectionActionBar
          ...
          onDraftTask={() => beginOrganizerDraft("task", selectedMessages)}
          onDraftDocument={() => beginOrganizerDraft("document", selectedMessages)}
          onDraftEvent={() => beginOrganizerDraft("event", selectedMessages)}
        />
      )}
      ...
      />
      <OrganizerDraftReviewSheet
        draft={draftReview}
        busy={draftSaving}
        onClose={() => setDraftReview(null)}
        onDiscard={(draft) => {
          const discarded = transitionOrganizerDraftReviewState(draft, "discarded");
          setDraftReview(null);
          void addMessage({
            role: "system",
            content: `Discarded ${discarded.draftType.replace(/_/g, " ")}.`,
          });
        }}
        onAccept={saveDraft}
      />
