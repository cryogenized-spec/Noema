...
export function CalendarScreen() {
  ...
  const [openIntelligenceOnEditorMount, setOpenIntelligenceOnEditorMount] = useState(false);
  ...
  const openCreate = () => {
    ...
    setOpenIntelligenceOnEditorMount(false);
  };
  ...
  const openEdit = (event: CalendarEventRecord, options?: { openIntelligence?: boolean }) => {
    setCreating(false);
    setEditingEvent(event);
    setOpenIntelligenceOnEditorMount(Boolean(options?.openIntelligence));
  };
    ...
    <section className="flex h-full min-h-0 flex-col gap-3 pb-16">
        ...
        <div className="fixed inset-0 z-40 flex items-end bg-black/45" onClick={() => setActionEvent(null)}>
          <div className="w-full rounded-t-2xl border border-noema-border bg-noema-panel p-3 pb-6 shadow-glass" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <button type="button" className="rounded-lg border border-violet-300/35 px-2 py-2 text-violet-100" onClick={() => { openEdit(actionEvent, { openIntelligence: true }); setActionEvent(null); }}>Intelligence actions</button>
      ...
      {(creating || editingEvent) && (
        <CalendarEventEditorSheet
          event={editingEvent}
          allEvents={events}
          openIntelligenceOnMount={openIntelligenceOnEditorMount}
          onClose={() => {
            setCreating(false);
            setEditingEvent(null);
            setOpenIntelligenceOnEditorMount(false);
