...
- While multi-select is active:
  ...
  - use the selection action bar for copy/delete/convert-to-note and organizer draft conversion (task/document/event)
...
- Current document capabilities:
  ...
  - document intelligence actions (extract tasks/events, summarize, suggest tags/title, identify actionable items) with review-first accept/discard flow
...
- Tasks now include intelligence actions (break down subtasks, rewrite clearly, summarize progress, estimate effort, suggest priority/next action/due direction, refine description) with draft-first review and explicit acceptance.
...
- Calendar edit/action flows now include **Calendar Intelligence** actions (refine, timing, duration, reminder, conflict scan, alternate slot) with a review-first sheet so proposals are editable and optional before applying.
...
## Organizer Intelligence (foundation)

- Added a shared `src/lib/organizer-intelligence/` foundation for draft-first organizer AI actions across chat/documents/tasks/calendar.
- Added typed invocation contexts:
  - `message_to_task`, `message_to_document`, `message_to_event`
  - `document_extract_tasks`, `document_extract_events`
  - `task_breakdown`, `task_schedule`, `event_refine`, `organizer_summary`.
- Added source-aware invocation contracts with entity type/id, excerpt, timestamp, invocation context, and selected-agent metadata.
- Added draft envelope contracts for review flows (`proposed`, `accepted`, `edited`, `discarded`) so AI suggestions are human-reviewed before writes.
- Added default organizer-agent routing groundwork via `useOrganizerIntelligenceStore` (local persisted default agent), with explicit agent override support.
- Added a unified **Organizer Review Queue** (`organizerReviewQueue` Dexie table + Zustand store + Organizer entrypoint) that persists pending AI proposals from chat/document/task/calendar intelligence and enforces accept/edit/discard review before writes.
- Deferred in this phase: autonomous background execution, cron/scheduler control, multi-agent orchestration, and direct auto-apply writes.

## Chat → Organizer conversion bridge

- Chat message actions now support organizer draft conversion for:
  - **Task draft**
  - **Document draft**
  - **Event draft**
- Conversion can start from:
  - single message action sheet
  - multi-select bulk actions.
- Drafts are generated as **review-first** envelopes and shown in a structured review sheet before any writes.
- Accepting a draft saves into the corresponding Organizer store (Tasks/Documents/Calendar); discarding makes no data changes.
- Source linkage is preserved via source metadata (`chatMessageIds` or chat-derived refs) on generated organizer drafts.
